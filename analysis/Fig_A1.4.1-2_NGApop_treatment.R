...
## Fig_A1.4.1-2_NGApop_treatment.R
##---------------------
...
prop_popU2_totalpop <- 0.068  ## From EMOD default demographics
require(readxl)
...
admin1_sp_disolv <- shapefile(file.path("data_files/nga_shp/nga_adm1_dissolved_ipti.shp"))
...
nga_pop <- read_xlsx(file.path('data_files', 'GEOPODE_NGA_population_ages0to100_version2022.xlsx'), skip = 6, trim_ws = T,
...
f_save_plot(pplot, plot_name = paste0('Fig A1.4.1'), width = 10, height = 6, plot_dir = plot_dir)


###-----------------------
### Fig A1.4.2
###-----------------------
dat <- fread(file.path('data_files', 'HS_by_LGA_v5.csv')) %>%
  mutate(NAME_1 = State) %>%
  filter(year == 2018) %>%
  group_by(NAME_1) %>%
...
pplot2 <- ggplot(data = dat) +
  ...
  labs(color = 'Population',
       x = 'State (ordered by treatment coverage)',
       y = 'Effective clinical\ntreatment coverage U5') +
...
f_save_plot(pplot, plot_name = paste0('Fig A1.4.2'), width = 10, height = 6, plot_dir = plot_dir)
