...
if (pmc_offset) {
  ...
  f_save_plot(pplot2, plot_name = paste0('Fig A1.1.0A'), width = 10, height = 4, plot_dir = plot_dir)
...
if (pmc_IIV) {
  ...
  print(pplot)
  f_save_plot(pplot, plot_name = paste0('Fig A1.1.0B'), width = 6, height = 3.5, plot_dir = plot_dir)
...
if (pmc_single_supp) {
  ...
  trial_PEs <- fread(file.path("data_files", "Aponte_2009_PMC_effectsize_studies.csv ")) %>%
    mutate(n_rounds = nchar(gsub("[^0-9]+", "", ipti_touchpoints_mth)))

  esu_PE <- fread(file.path("data_files", "Esu_2021_PMC_effectsizes_Cochrane.csv")) %>%
  ...
  print(pplot)
  f_save_plot(pplot, plot_name = 'Fig A1.1.1', width = 9, height = 6.5, plot_dir = plot_dir)
