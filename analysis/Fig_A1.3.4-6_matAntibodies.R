...
pplot <- ggplot(data = plotdat) +
  ...
  labs(y='% relative reduction', x = 'annual EIR (log scale)') +
  scale_y_continuous(expand = c(0, 0), breaks = seq(0, 0.5, 0.05), labels = seq(0, 0.5, 0.05)*100) +
...
f_save_plot(pplot, paste0('SI_fig_PMC_maternalantibodies'),
            file.path(plot_dir), width =8, height =4, units = 'in', device_format = device_format)


pplot <- dat_aggr_mth %>%
  filter(age <= 365 &
    Annual_EIR %in% c(8, 32,128) &
           pmc_coverage == 0 &
           name %in% c( 'clinical_cases', 'severe_cases')) %>%
  mutate(EIR = Annual_EIR,
         name = case_when(name == 'clinical_cases' ~ 'Clinical malaria',
  ...
  facet_wrap(name ~ EIR, nrow = 2, scales = 'free', labeller = labeller(EIR = label_both, name = label_value)) +
